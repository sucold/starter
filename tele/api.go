...
func (b *Bot) Raw(method string, payload interface{}) ([]byte, error) {
	...
	resp, err := b.Request(http.MethodPost, url, "application/json", &buf)
	if err != nil {
...
func (b *Bot) sendFiles(method string, files map[string]File, params map[string]string) ([]byte, error) {
	...
	url := b.URL + "/bot" + b.Token + "/" + method
	resp, err := b.Request(http.MethodPost, url, writer.FormDataContentType(), pipeReader)
	if err != nil {
		err = wrapError(err)
...
func (b *Bot) getUpdates(offset, limit int, timeout time.Duration, allowed []string) ([]Update, error) {
	...
	if timeout.Nanoseconds() != 0 {
		params["timeout"] = strconv.Itoa(int(timeout / time.Second))
	}
	if limit != 0 {
